...
from kyle_claude.core.config import KyleConfig, LlmConfig, get_config
...
_DEFAULT_CONFIG_PATH = Path("~/.kyle/config.toml").expanduser()
...
def _sync_project_dotenv(config: LlmConfig, previous_key_env: str) -> None:
    ...
    updates = {
        "KYLE_LLM_PROVIDER": normalize_provider(config.provider),
        "KYLE_LLM_DEFAULT_MODEL": config.default_model,
        "KYLE_LLM_BASE_URL": config.base_url,
        "KYLE_LLM_API_KEY_ENV": config.api_key_env,
    }
...
# 打印当前配置状态，隐藏密钥正文
def print_llm_status(config: KyleConfig) -> None:
...
def cmd_configure(config: KyleConfig) -> None:
    if not sys.stdin.isatty():
        raise SystemExit("Interactive terminal required for `kyle configure`.")
    updated = configure_llm(config)
    from kyle_claude.cli.commands.core import ensure_core_running, stop_core
