...
| P0 | main ruleset | 远端审计明确报告未启用 | 在 GitHub 仓库设置中要求稳定 CI/Security 汇总检查，再运行一次远端审计 |
...
| P2 | 目录遗留命名 | 根目录仍有历史 `specdriven.md` 和诊断脚本 `trace_permission_flow.py` | 下次确认引用后移入 `docs/` 与 `scripts/`，同步链接和脚本说明；不做无验证的仓促移动 |
...
5. 一组变更完成后统一运行本地门禁、提交并推送，减少 Actions 邮件和重复等待。
