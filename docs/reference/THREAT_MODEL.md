...
| TM-10 | 崩溃导致 ledger 尾部损坏、孤儿 tool call、SQLite 投影漂移或审计记录静默丢失 | checksum chain、尾部恢复、SQLite 投影、runtime doctor/reconcile、turn 终态配对；ledger/projection 写失败触发 `audit.degraded` 并停止非 READ 工具 | 仓库内失败关闭机制 **支持**；当前公开 Beta 的三平台 100 次强杀外部报告尚未产生，见评分卡 |
...
| TM-12 | 依赖、workflow 或发布产物被供应链污染 | workflow 最小权限、Dependabot、快速 required CI、手动重型矩阵、Release SBOM/checksum/provenance/signature 流程 | **部分支持**；首个手工 Beta 有 checksum、manifest 和 contract，但没有 dependency-review required job、workflow provenance、SBOM 或签名；远端 ruleset/Actions 状态仍须单独验证 |
