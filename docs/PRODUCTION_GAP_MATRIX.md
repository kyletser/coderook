...
| R1 | SandboxBackend 与 Linux/macOS 真实边界 | 完成实现 | `probe/plan/spawn/describe` 后端契约；shell/Run/background/fleet 共用 spawn 计划；receipt 持久完整计划；真实文件、symlink、子进程和网络负例脚本；域白名单不可强制时在 spawn 前 fail closed，绝不扩大为全网 | 本机仅验证 Windows degraded；最新远端 CI 在前置测试失败后跳过边界脚本；按域正向放行仍需可接受的 OS 强制后端 |
...
| R2 | 故障注入 | 候选门禁已实现 | 100 个均匀截断点；`run_crash_recovery_matrix.py` 真实阻塞模型请求、强杀、重启并重建 receipt；鉴权 API 就绪探针与 GET 瞬时重试消除重启竞态；本机 5/5 smoke | 三平台各 100 次 workflow 报告尚未产生 |
...
| R4 | VS Code 扩展 | 完成打包候选 | HTTP/SSE、创建/恢复、发送、审批/逐 hunk、diff、steer/interrupt；`npm run check` 与 `npm run package` 本机通过；CI 上传 VSIX | 真实 daemon Extension Host UI 冒烟与 Marketplace 发布 |
...
| R5 | wheel/Docker/Windows 安装与 portable | 完成候选流水线 | Dockerfile/compose、安装脚本、内置 Python portable 构建脚本；`distribution.yml` 产出三平台 wheel、Docker smoke、Windows portable 与 VSIX；本机 wheel smoke 通过 | distribution workflow 实际报告、签名 Windows 安装包和跨已发布版本升级报告 |
| R5 | 三平台 CI | 修复待复验 | Ubuntu/Windows/macOS matrix、sandbox、benchmark contract、Linux Mypy contract、VSIX；本机完整 gate 通过 | CI #31 三平台在测试阶段失败；当前跨平台测试修复需远端新 run 变绿 |

## 当前发布结论

仓库内可实现的主要改造已经覆盖 R0–R5，但 **CodeRook 仍不满足公开 Beta 门禁**。本机完整 gate 已通过，远端三平台 CI 的已知测试失败已在当前候选修复但尚未复验。shell 域名级出站策略已保证无法强制时 fail closed，但按域正向放行仍没有可接受的 OS 后端，不能冒充完成。剩余门禁包括真实模型 pass@1、Linux/macOS 安全矩阵、三平台各 100 次 daemon 强杀、候选安装包/容器实装和官方 MCP 兼容报告。最新结论以 `RELEASE_SCORECARD.md` 为准。
