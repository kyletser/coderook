...
仓库内可实现的主要改造已经覆盖 R0–R5，但 **CodeRook 仍不满足公开 Beta 门禁**。三平台连续 CI、Security、Distribution、官方 MCP、逐平台沙箱 JSON、资源基线与三平台各 100 次恢复均已有远端成功记录；剩余门禁是固定真实模型 pass@1/成本、公开 benchmark 官方判分、active main ruleset、跨已发布版本升级和首次可验证 Release。shell 域名策略仍只支持无法强制时 fail closed，不能冒充按域正向放行。最新结论以 `RELEASE_SCORECARD.md` 为准。
